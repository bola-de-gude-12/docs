There's a significant difference between running Document Capture in the on-premises version of Microsoft Dynamics 365 Business Central and running it in the online version. This article describes the minimum requirements and things to note when running Document Capture in Business Central online. For the on-premises requirements, see [Minimum requirements for using Continia Document Capture on premises](@DC-135).

> [!IMPORTANT]
> As Document Capture is an integrated part of Business Central, the [minimum requirements for Business Central](https://learn.microsoft.com/en-us/dynamics365/business-central/product-requirements) (Microsoft article) also apply to Document Capture.
...
## Email requirements
You can import documents into Document Capture by sending them via email to a predefined email address hosted by Continia. See [Additional guidelines for using Continia Document Capture](@DC-452) for technical requirements and things to note.
...
The version of the PDF file must be PDF 1.7 or earlier, and it must conform to the PDF/A standard. Handwritten text isn't supported, and is unlikely to be recognized well.
...
However, none of the previous versions of the Web Approval Portal support the simultaneous signing in of users from different databases. If your organization has more than one database in Business Central and exports users from one of these databases to the Web Approval Portal, only those newly exported users are able to sign in to the Web Approval Portal. All existing users previously exported from another database will be deleted from the Web Approval Portal and unable to sign in.

> [!NOTE]
> Security filters are currently not supported in the Web Approval Portal. To control what's visible and accessible to users in the portal, use account and dimension permissions instead. These can be configured for [individual users](/continia-document-capture/setting-up-document-capture/setting-up-document-approval/continia-user-setup-for-approvals#configuring-account-and-dimension-permissions) or for [groups of users](/continia-document-capture/setting-up-document-capture/setting-up-document-approval/approval-user-groups#to-configure-account-and-dimension-permissions-for-a-group).
