---
title: Creating contract invoices in Document Capture
date: 23-07-2025
...
You can automate the linking process using the posting setup, which enables you to decide how amounts are posted for incoming invoices when they're registered. By specifying that the amounts must be transferred from a specific purchase contract for all invoices using the selected template, you enable Document Capture to automatically create a link between the invoices and the specified contract during invoice registration. To do this:
...
1. On the action bar, click **Home** > **Register** to register the invoice.
