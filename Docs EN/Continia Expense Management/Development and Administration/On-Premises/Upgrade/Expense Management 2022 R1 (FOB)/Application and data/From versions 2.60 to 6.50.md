...
When upgrading from Document Capture 7.00 and/or Expense Management 7.00 to version 8.00, use the regular upgrade procedure described in [Manual Upgrade from Version 7.00 to Version 8.00](@EM-30) or [Automated Data Upgrade from Version 7.00 to Version 8.00](@EM-31).
...
Ensure the current system is running one of the supported versions of Document Capture or Expense Management listed in the summary section. If not, it is very important that you follow the upgrade guide from an earlier version to DC4.50 and or EM2.60. You can find previous upgrade guides here:
...
You must perform the upgrade process using a developer license. After the upgrade, ensure you use a new/updated customer license file from Microsoft and import it into NAV. 
...
> [!NOTE]
> This task is not relevant if you're upgrading from DC6.00.x or DC6.50.x or if you're upgrading from EM4.00.x or EM6.50.x.

Before starting the upgrade, make sure the product activation status is correct in all companies that are being upgraded.
...
Running this page/form (depending on the NAV version) will allow you to activate or deactivate Document Capture and Expense Management separately in each company. The upgrade can not continue unless all product/company combinations are either Activated or Deactivated.

## Task 5: Check pre-upgrade package

The pre-upgrade package depends on both the Document Capture and/or Expense Management version you are upgrading from, and the NAV/Business Central version.

Ensure you use the correct upgrade package.
...
After importing the correct pre-upgrade package (replace all), go to either **Document Capture Setup** or **Expense Management Setup** and select **Upgrade Upgrade Data to Latest Version**. This performs the pre-upgrade.

> [!IMPORTANT]
> Only start the upgrade in a company with either Document Capture or Expense Management activated.
...
> [!NOTE]
> The installer will update the add-ins folder (Client and/or Server) on the standard installation path. Therefore, if your installation path differs from the standard Microsoft path, make sure you copy the new add-in folder to the folder where your installation resides.
...
> [!IMPORTANT]
> Before doing object modifications or starting the data upgrade, make sure you back up the database.
...
> [!WARNING]
> For all versions from NAV 2013 to Business Central October 2018 (BC v13), the following applies: When importing the DC8.00/EM8.00 object package, skip the four pages mentioned below. The pages were unintentionally included in the release packages – they will be removed in service pack 1.
>
>* Page 5 **Currencies**
...
1. Import the object post-upgrade package corresponding to your NAV/Business Central version and the Document Capture/Expense Management version that you're upgrading from (e.g: “NAV 2016 to BC14 - DC4.50 to DC8.00, EM2.60 to EM8.00 – Direct Upgrade Post.fob”).
...
1. Compile all Document Capture and Expense Management objects (Version List filter \*DC\*|\*EM\*|\*CC\*|\*DN\*). When using NAV 2015 or a newer version, set Synchronize Schema to “Later”. Some Document Capture/Expense Management objects will not compile and will be deleted later in the upgrade process.
1. Compile all MenuSuites (not only Document Capture and Expense Management).
...
| NAV RTC Clients 2009 -> 2016 | Only perform the following steps if you are using Dynamics NAV 2009 -> 2016: <ol><li>Uninstall &ldquo;Document Capture RTC Client Components&rdquo;.</li><li>Install &ldquo;Document Capture RTC Client Components&rdquo;</li></ol> |
...
Some objects that were part of the Document Capture and Expense Management version that you upgraded from are not needed anymore. Therefore, whenever you upgrade from one of the versions below, delete the objects specified:
...
Document Capture contains modifications to the following four standard pages. These modifications should be removed manually after the upgrade:
