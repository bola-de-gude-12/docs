...
date: 18-09-2025
description: The minimum requirements and recommendations for using Document Capture in Business Central on premises.
id: DC-135
lang: en
---
...
> [!IMPORTANT]
> When using the RTC client, you must install a version of the scanner driver that corresponds to the Microsoft Dynamics NAV version. For example, if you have the 32-bit version of Microsoft Dynamics NAV installed, you must install the 32-bit version of the scanner driver.
...
> [!NOTE]
> ABBYY and the Document Capture Service don't support high availability/load balancing. If this is important to you, consider using Continia Cloud OCR instead, as this is designed for high availability.
...
## Email requirements
You import documents into Document Capture simply by sending them via email to a predefined email address hosted by Continia. See [Additional guidelines for using Continia Document Capture](@DC-224#technical-email-requirements) for technical requirements and things to note.
...
> [!NOTE]
> IMAP support for Exchange Online will be discontinued. For more information, see [Extension of the OCR service to support Exchange Web Services (EWS) instead of IMAP](@DC-373).

## Continia Web Approval Portal
See [Requirements for using the Continia Web Approval Portal on premises](@DC-454). 
...
| eun-cdc.continiaonline.com | 443 | Outgoing | Production |
| democontiniaonline.com | 443 | Outgoing | Test |
| ocsp.sectigo.com | 443 | Outgoing | Both |
...
| Address  | Port(s)  | Direction  | Environment |
| --- | --- | --- | :-: |
| registration2.abbyy.com<a href="#footnote-1"><sup>1</sup></a> | 80, 443 | Outgoing | N/A |
| Your email server | 143 (unsecured IMAP) or 993 (secure IMAP)<a href="#footnote-2"><sup>2</sup></a> | Outgoing | N/A |
...
<div class="footnotes">
  <hr />
  <ol>
    <li id="footnote-1">The ABBYY activation server is located in Microsoft Azure (The Netherlands).</li>
    <li id="footnote-2">Please refer to our general documentation on this. You can find this in the Continia Document Capture software package, in the <b>Documentation</b> folder.</li>
  </ol>
</div>
</small>
...
The Continia File Service is built on .NET Core 8, which is supported on Windows Server Core 2012 or later. If you have an earlier version of .NET Core, make sure to update it to .NET Core 8.

## Additional guidelines 
A number of other requirements and recommendations are relevant to mention, as they have an impact on how well Document Capture performs and how the app is used. For more information, see [Additional guidelines for using Continia Document Capture](@DC-224).
