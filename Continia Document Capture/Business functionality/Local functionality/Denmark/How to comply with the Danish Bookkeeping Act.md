...
> [!NOTE]
> This is the English version of the article. You can find the Danish version here: [Sådan kan du overholde bogføringsloven](@DC-170).

> [!IMPORTANT]
> Support for the Danish Bookkeeping Act has been fully implemented in Continia's solutions as of version 12.02.
...
If an organization stores purchase documents that have been registered using Document Capture or Expense Management in the Business Central database, the documents will appear in the database twice. This method is a convenient and simple, but for some organizations it may take up a disproportionately large amount of database space, as the documents are stored in both Continia's tables and Microsoft's **Incoming Documents** table. Such organizations can choose to store their purchase documents from Document Capture and Expense Management using Azure Blob Storage instead. For more information, see [Setting up Azure Blob Storage](@DC-171).
