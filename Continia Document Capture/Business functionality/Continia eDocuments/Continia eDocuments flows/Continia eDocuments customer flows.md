...
lang: en
description: >-
  How to send purchase orders and invoice responses, register order responses,
...
For information on the corresponding vendor flows, see [eDocuments vendor flows](../../../../Continia%20Document%20Capture/Business%20functionality/Continia%20eDocuments/Continia%20eDocuments%20flows/@DC-221/).
...
2. On the action bar, click **Print/Send** > **Send** to open the **Send Document to** dialog box.
3. In the **Electronic Document** dropdown list, select **Through Continia Delivery Network**. Click **OK** to confirm and then **OK** again to return to the **Purchase Order** page.
4. On the **General** FastTab, check [the **eDocument Status** field](../../../../Continia%20Document%20Capture/Business%20functionality/Continia%20eDocuments/Continia%20eDocuments%20flows/@DC-183/#the-edocument-status-field):
   ...
   * **Not Valid** - click this to open the **eOrder Document** page, and then do as follows:
     ...
     3. When done, click **Resend** on the action bar to send the revised purchase order to the vendor.

The purchase order has now been sent to the vendor, who can process it as described in [To import and register a sales order](../../../../Continia%20Document%20Capture/Business%20functionality/Continia%20eDocuments/Continia%20eDocuments%20flows/@DC-221/#to-import-and-register-a-sales-order).
...
2. The order response is now ready to be registered and available in the **Ready to Register** cue. Click this to open the document journal.
3. On the action bar, click **Document** > **Document Card** to open the document card.
4.  If the vendor has made any line changes, these are displayed on the **Lines** FastTab.

    > \[!NOTE] Only the actual changes are displayed here – no other line details relating to quantity, price, unit of measure and similar are visible. To see the full context of the line changes (before and after), click **Preview Order Changes** on the action bar to open the **Purchase Order Changes Preview** page.
...
6. On the action bar, click **Home** > **Register** to register the order response.
...
> \[!NOTE] Only one change can be submitted per document. If you or the vendor submits a price or quantity change, for example, the other party can't submit any other changes.

> \[!TIP] For an overview of the changes made by the vendor, click **Home** > **Match Lines** on the action bar from either the document card or the document journal.
...
2. On the action bar, click **Actions** > **eDocuments** > **Update**.
...
> \[!NOTE] Although a cancellation submitted by the vendor is final, cancellations submitted by a customer can be either accepted or rejected by the vendor.
...
1. In the Role Center, under **Continia Document Capture Activities**, go to **Actions** and click **Import Files**.
2. The purchase invoice is now available in the **Ready to Register** cue. Click this cue to open the document journal.
3. On the action bar, click **Document**.
   ...
   2. If the purchase invoice includes invoice lines and you need to set up translations, for example, click **Document Card** to open the document card.
4. The document card allows you to do everything you're used to from Document Capture, including [document matching](../../../../Continia%20Document%20Capture/Business%20functionality/Continia%20eDocuments/Continia%20eDocuments%20flows/@DC-61/). To carry out matching, select **Home** > **Match Lines**. Make any necessary adjustments to complete the matching process.
5. When you're done with the purchase invoice and no warnings are displayed, click **Register** on the action bar to register the purchase invoice.
6.  The **Purchase Invoice** page opens. From here, you can manually notify the vendor that you've accepted, rejected, or paid the invoice – or that you're processing it or have questions.

    1. On the action bar, click **Invoice** > **Send Electronic Confirmation**.
    2. In the dialog that opens, select the appropriate option – for example, **Accepted** – and then click **OK** to close the dialog.
    ...
    5. On the action bar, click **Send** to send your response to the vendor.
    ...
    3. **Send In Process** - choose when to notify that the invoice is being processed. For example, when releasing the document.
    4. **Send Accepted** - choose when to notify that the invoice has been accepted. For example, when importing the document.
    5. **Send Rejected** - choose when to notify that the invoice has been rejected.

## Related information

[eDocuments flows](../../../../Continia%20Document%20Capture/Business%20functionality/Continia%20eDocuments/Continia%20eDocuments%20flows/@DC-183/)\
