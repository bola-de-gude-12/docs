...
3. In the document list, select the document you want to register. Select the document line – not the number in the **No.** column, as this opens the document card.
4. In the document fields section (under **Document Header**), check that all fields in the **OK** column have a check mark. If so, all fields are considered valid. If not, follow the field capturing guides in the [To capture fields](../../../Continia%20Document%20Capture/Business%20functionality/Documents%20and%20templates/@DC-110/#to-capture-fields) and [Training Document Capture to locate values](../../../Continia%20Document%20Capture/Business%20functionality/Documents%20and%20templates/@DC-110/#training-document-capture-to-locate-values) sections of [Capturing header fields in a document](../../../Continia%20Document%20Capture/Business%20functionality/Documents%20and%20templates/@DC-110/) to ensure that all fields are captured correctly and have valid values.
5. Not all field values are captured from the document – some must be added manually instead. The most important is the general ledger account number, which in most cases is mandatory. To add this value, go to **No.** and select the three dots in the **Value** column to open the **G/L Account List**. Choose a relevant account in the list by selecting its number in the **No.** column.
6. A dialog box appears, asking if you want to configure the selected account as the default. Select **Yes** if you want Document Capture to assign this account to all documents using the same source template – otherwise select **No**. Your response closes the dialog box and adds the selected value to the **No.** field.
...
As an alternative to the standard registration setup, it's possible to register documents as either general journal lines or purchase journal lines, but without vendor names and other standard details. If you [set this up](../../../Continia%20Document%20Capture/Business%20functionality/Documents%20and%20templates/@DC-112/), Document Capture automatically creates lines directly in the general or purchase journal upon document registration – instead of creating invoices or credit memos.
...
*   Following registration, the **Edit - General Journals** page opens instead of the document page because no document is created.
    ...
    > \[!NOTE] This page opens if you've kept the default settings on the **Template Card** – that is, if **Show Document After Register** is set to **Always** and if **Invoice Reg. Step 2** and **Credit Memo Reg. Step 2** are left empty.
* In the document journal, when you select **Document** > **Find Entries** on the action bar, one of the following happens, depending on whether you've posted the general journal (for example, by setting **Invoice Reg. Step 2**/**Credit Memo Reg. Step 2** to **Post** in [the setup](../../../Continia%20Document%20Capture/Business%20functionality/Documents%20and%20templates/@DC-112/)):
...
> \[!TIP] To see registered documents in their original form, use the document viewer on the **General Journals** page.
...
3. In the document list, select the invoice you want to register as a prepayment invoice. Select the document line – not the number in the **No.** column.
4.  In the document fields section (under **Document Header**), go to **Our Order No.** and check that an order number is displayed in the **Value** column. If not, select the three dots on the right to open the **Purchase List**, and then select the order that you want to link to the invoice.
    ...
    > \[!NOTE] A purchase order number must be entered or captured for Document Capture to register the invoice as a prepayment invoice.
...
6. [If something isn't correct, one or more error comments are displayed](<Registering documents.md#automatic-validation-and-updating-of-the-prepayment-percentage>) in bold red in the **Comments** section at the bottom. Resolve these errors to enable document registration.
7. When no more error comments are displayed, register the invoice by selecting **Home** > **Register** on the action bar.
...
_No prepayments have been processed on purchase order \[order number]. The current prepayment invoice will increase the prepaid percentage and amount to \[percentage (amount)]_.

> \[!TIP] Although this comment doesn't by default prevent registration, you can configure it to do so by changing it from a warning to an error. For more information, see [Configuring comment types and importance](../../../Continia%20Document%20Capture/Business%20functionality/Documents%20and%20templates/@DC-72/).
...
| <p>Document<br><br></p> | Amount excl. tax |        Prepayment %        | <p>Comments<br><br></p>                                                                                                                                                                                                                                                                                                                                          |   |
| ----------------------- | :--------------: | :------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | - |
| Purchase order          |      $5,000      |  <p>10%<br>(original)</p>  | As the amount of the invoice is one fifth of the purchase order amount – corresponding to 20% –, the original prepayment percentage of 10% is adjusted accordingly. A warning about the mismatch is displayed in the document journal before document registration. After registration, Document Capture changes **Prepayment %** to 20% for the purchase order. |   |
| Prepayment invoice      |      $1,000      | <p>20%<br>(equivalent)</p> |                                                                                                                                                                                                                                                                                                                                                                  |   |
...
| <p>Document<br><br></p> | Amount excl. tax |         Prepayment %        | <p>Comments<br><br></p>                                                                                                                                                                                                                                                          |   |
| ----------------------- | :--------------: | :-------------------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | - |
| Purchase order          |      $5,000      |    <p>10%<br>(actual)</p>   | As the amount of the invoice exceeds the purchase order amount – corresponding to 120% – the original prepayment percentage of 10% won't be adjusted. An error message about the amount mismatch is displayed in the document journal, preventing the invoice from registration. |   |
| Prepayment invoice      |      $6,000      | <p>120%<br>(equivalent)</p> |                                                                                                                                                                                                                                                                                  |   |

> \[!NOTE] All Document Capture prepayment functionality applies at header level, as **Prepayment %** is a purchase order header field. It's not possible to link prepayment invoices to purchase orders at line level.
