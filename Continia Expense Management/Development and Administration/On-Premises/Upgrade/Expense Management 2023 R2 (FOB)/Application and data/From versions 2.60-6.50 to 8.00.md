...
date: 31-09-2023
description: A guide on how to upgrade directly from versions 2.60–6.50 to version 8.00
lang: en
---
...
As regards migration from a FOB-based version of NAV/Business Central to an on-premises extension and/or online, see [Upgrading NAV/Business Central with Expense Management Installed](@EM-107) and [Migrating Expense Management from Business Central On-Premises to Cloud](@EM-112).
...
* “NAV 2016 to BC14 - DC4.50 to DC8.00, EM2.60 to EM8.00 – Direct Upgrade **Pre**”.fob
   ...
   * The Document Capture/Expense Management version before the upgrade is DC v4.50 and EM v2.60
   * This is the pre-upgrade .fob

## Task 5: Run pre-upgrade step

After importing the correct pre-upgrade package (replace all), go to either **Document Capture Setup** or **Expense Management Setup**, and then select **Upgrade Upgrade Data to Latest Version**. This will perform the pre-upgrade.
...
> 
> Before starting the upgrade, make sure the product activation status is correct in all companies that are being upgraded. If the upgrade code detects a company without a valid activation state, it will stop with the following error message:
> 
...
> [!NOTE]
> The installer will update the add-ins folder (Client and/or Server) in the standard installation path. Therefore, if your installation path differs from the standard Microsoft path, make sure you copy the new add-in folder to the folder that your installation is in.
...
1. Compile all Document Capture and Expense Management objects (Version List filter \*DC\*|\*EM\*|\*CC\*|\*DN\*). When using NAV 2015 or a newer version, set Synchronize Schema to “Later”. Some Document Capture/Expense Management objects will not compile. They'll be deleted later in the upgrade process.
1. Compile all MenuSuites (not just Document Capture and Expense Management).
...
| NAV RTC Clients 2009 – 2016 | Only perform the following steps if you're using Dynamics NAV 2009 – 2016: <ol><li>Uninstall &ldquo;Document Capture RTC Client Components&rdquo;.</li><li>Install &ldquo;Document Capture RTC Client Components&rdquo;</li></ol> |
...
Document Capture contains modifications to the four standard pages mentioned below. These modifications should be removed manually after the upgrade.
