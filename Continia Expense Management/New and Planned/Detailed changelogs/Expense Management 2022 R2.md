...
| Expenses|We have fixed an issue when detaching an allocated expense from an Expense Report that was causing the following error:<br><ul><li><em>This expense has been allocated and therefore you cannot change Amount.<br></em> </li> </ul> |44036|
...
| <span style="white-space: nowrap;">Functional area</span> | Description | ID |
| --- | --- |  --- |
| Expenses | When trying to "Get Expenses" from an Expense Report, when the expense would have been matched, the following error was shown:<br><ul><li><em>Matched to Bank Transaction must be equal to 'No' in Expense: Entry No.=<EntryNo.>. Current value is 'Yes'.</em></ul></li> | 42497 |
...
| Expenses|<span>To delete a Payment Type is now an option, as long as no un-posted documents are using that Payment Type. |40335|
