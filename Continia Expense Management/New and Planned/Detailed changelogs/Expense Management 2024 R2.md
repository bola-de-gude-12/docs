...
| Expenses | When using <b>Expense Management</b> in combination with <b>Continia Sustainability</b>, it is now mandatory to specify an <b>Emission Unit Code</b> on an expense if an <b>Emission Quantity</b> is provided. | 65797 |
...
| General Application|<span>In a localized version of Business Central, the following error would have prevented users from posting expense documents where sustainability accounts were specified:</span> <ul><li><em>The &lt;Environmental Account No.&gt; must be of type Posting.</em></li></ul> |64846|
...
| Expenses|The following event has been added to R<span>eport</span><span>&nbsp;</span><span>CEM Export Attachments:</span> <ul><li>OnExportAttachmentsOnCreateFileName </li></ul> |61796|
...
| General Application|<span>The following</span><span>&nbsp;</span><span>event publisher</span><span>&nbsp;</span><span>has been added to</span><span>&nbsp;</span><span>table 6086333</span><strong> </strong>CEM Exp. Posting Desc. Field<span>&nbsp;</span><span>to enable customization of the</span><span>&nbsp;</span><b>Posting Description</b><span>&nbsp;</span><span>field when posting Expense Management documents:</span> <ul><li>OnBeforeReplacePlaceholdersWithValues</li></ul> |63479|
...
| Expenses| When installing <b>Continia Expense Management</b> and <b>Continia Sustainability</b> in production, demo data is no longer created. The relevant <b>Sustainability</b> setup is now managed through the configuration file. |60601|
...
| Expenses| With <b>Continia Sustainability</b> installed, expenses no longer get stuck in the inbox due to certain setup combinations. Now, an error appears if an expense is posted with an invalid environmental account. |63604|
...
| General Application|<span>Upgrading a company where <b>Digital Sign Attachments</b> is enabled now requires a <b>Company Code</b> in <b>Continia Company Setup</b> to prevent the following error:<ul><li><em>&quot;The remote party closed the WebSocket connection without completing the close handshake.&quot;</em></li></ul>  |62215|
...
| <span style="white-space: nowrap;">Functional area</span> | Description                                                  | ID    |
| --------------------------------------------------------- | ------------------------------------------------------------ | ----- |
| General Application                                       | When importing the Expense Management configuration file, the following error would have been preventing the user to succeed:<li>The Field Type \<Field Code\> is expected to have lookup values. Please check the configuration of the field.</li> | 62056 |
...
| Document Approval|<span>Event Publisher has been added to Codeunit&nbsp;CEM Approval Management:</span><ul><li><span>OnBeforeGetNextApprover</span> </li></ul> |61006|
...
| General Application|Applying a <b>Fixed Filter</b> on the <b>Field Type Source Table</b> with a length exceeding 100 characters would previously result in the following error: <ul><li>The length of the string is &lt;filter length&gt;, but it must be less than or equal to 100 characters. Value: &lt;filter value></li></ul> |61230|
