---
title: Onboarding ABN AMRO with Direct Communication
...
To establish direct communication between Payment Management and ABN AMRO, you must register at ABN AMRO, set up the banking API agreement, and obtain a TLS certificate. 
...
To establish direct communication between Payment Management and ABN AMRO, you must set up a banking API agreement on the services **Business Account Payment API** and **Business Account Insight API**. 

To request production access:
1. Log in and select the stack icon on the top right side of the webpage.
   ![Stack icon ABN AMRO developer portal](/images/PM365/ABN-production-access.png)
2. Select **Request production access** > **Account APIs**. 
3. Select the **Business Account Insights** and **Business Account Payments**.
   
   ![Account APIs](/images/PM365/ABN-production-access-selection.png)
...
8. Under **API Usage**, enter the following information:
   ...
   * **Use case** - enter *Continia Software – Payment Management*. 
9. Click **Preview**, verify all the info is correct, and select **Submit**. Once you have submitted the request for production access, the ABN AMRO support team will send a contract to the contact person listed on the form. After signing the contract, it will take ABN AMRO approximately a week to establish the connection. 
   ...
   * **API key** - go to the [ABN AMRO Developer Portal](https://developer.abnamro.com/) and select the stack icon. Navigate to the **Operations** section and select **View**. Go to **Credentials** and the **API key** field. This key is required for setting up the connection in Payment Management. 
     ![ABN API key](/images/PM365/ABN-api-key.png)
...
| Client ID            | Your username for the ABN AMRO API. This ID is required for setting up the connection in Payment Management. | Sent to you in an email by ABN AMRO. See [To request production access](#to-request-production-access) for more details. |
| API key              | The key for the ABN AMRO API. This key is required for setting up the connection in Payment Management. | You can find the **API key** by selecting the stack icon. Navigate to the **Operations** section and select **View**. Go to **Credentials** > **API Key**. See [To request production access](#to-request-production-access) for more details. |
...
1. On the **Bank System** field, select ABN AMRO and select **OK**.
